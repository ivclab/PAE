...
_MASK_COLLECTION = 'masks'
_THRESHOLD_COLLECTION = 'thresholds'
_WEIGHT_COLLECTION = 'kernel'
_MASKED_WEIGHT_COLLECTION = 'masked_weights'
...
def _update_weight_according_to_new_mask(weight, new_mask):
  zero_out = tf.constant(value=0.0, shape=weight.shape, dtype=weight.dtype)
  return weight.assign(tf.where(tf.equal(new_mask, 0), zero_out, weight)) 

def have_encountered_new_task(mask, task_id=1):
  return tf.reduce_any(tf.equal(mask, task_id), name='have_encountered_new_task')

def initialize_mask_and_weight(mask, weight, task_id=1):
  task_id_mask = tf.constant(value=task_id, shape=mask.shape, dtype=mask.dtype)
  random_weight = tf.random_normal(shape=weight.shape, dtype=tf.float32)
  if task_id == 1:
    return tf.cast(mask.assign(tf.where(tf.equal(mask, 0), task_id_mask, mask)), dtype=tf.bool)
  else:
    return tf.logical_and(
        tf.cast(weight.assign(tf.where(tf.equal(mask, 0), random_weight, weight)), dtype=tf.bool),
        tf.cast(mask.assign(tf.where(tf.equal(mask, 0), task_id_mask, mask)), dtype=tf.bool)
    )
...
def apply_mask(x, scope, task_id=1):
  ...
  conditional_op = control_flow_ops.cond(have_encountered_new_task(mask, task_id),
...
class Pruning(object):
  ...
  def __init__(self, task_id=1, spec=None, global_step=None, sparsity=None):
    ...
    # Every new task should be its own pruning object
    self._task_str = 'task_' + str(task_id)
  ...
  def _setup_last_update_step(self):
    with variable_scope.variable_scope(self._spec.name):
      with variable_scope.variable_scope(self._task_str):
        try:
  ...
  def _update_mask(self, weights, threshold, old_mask):
    ...
    with ops.name_scope(weights.op.name + '_pruning_ops'):
      ...
      # only the weights that belong to current task will be draw out, 
      # and store into one-dim list
      boolean_mask = tf.logical_or(
          tf.equal(old_mask, 0),
          tf.equal(old_mask, self._task_id))
      boolean_masked_weights = tf.boolean_mask(abs_weights, boolean_mask)  
      max_value = math_ops.reduce_max(boolean_masked_weights)
      histogram = _histogram(
          boolean_masked_weights, [0.0, max_value],
      ...
      num_bins = math_ops.reduce_sum(math_ops.cast(
          math_ops.less(norm_cdf, self._sparsity), np.float32))

      num_zeros = tf.subtract(tf.cast(tf.size(boolean_mask), tf.int64), tf.count_nonzero(boolean_masked_weights))

      def true_fn(): return 1.0
      def false_fn(): return num_bins

      # means there is too many uninportant params (more than the rate of limited sparsity)
      should_force_to_prune = tf.logical_and(
          tf.greater_equal(self._global_step, int(self._spec.end_pruning_step * 0.75)), # if reach the whole 75% of pruning process
          tf.equal(num_zeros, 0))                                                       # and never been prune before

      num_bins = tf.cond(should_force_to_prune, true_fn, false_fn)
      ...
      smoothed_threshold = current_threshold
      # smoothed_threshold = math_ops.add_n([
      #     math_ops.multiply(current_threshold, 1 - self._spec.threshold_decay),
      #     math_ops.multiply(threshold, self._spec.threshold_decay)
      # ])        
      zero_mask = tf.constant(0, dtype=old_mask.dtype, shape=old_mask.shape)

      new_mask = tf.where(
          tf.logical_and(
              tf.equal(old_mask, self._task_id),
              tf.less_equal(abs_weights, smoothed_threshold)
          ),
          zero_mask,
          old_mask
      )
  ...
  def _maybe_update_block_mask(self, weights, threshold, old_mask):
    ...
    if squeezed_weights.get_shape().ndims != 2 or self._block_dim == [1, 1]:
      return self._update_mask(weights, threshold, old_mask)
  ...
  def _get_mask_assign_ops(self):
    ...
    for index, mask in enumerate(masks):
      ...
      new_threshold, new_mask = self._maybe_update_block_mask(
          weight, threshold, mask)

      self._assign_ops.append(
          _update_weight_according_to_new_mask(weight, new_mask))
  ...
  def inspect_maybe_update_masks(self):
    with ops.name_scope(self._spec.name):
      is_step_within_pruning_range = math_ops.logical_and(
          math_ops.greater_equal(self._global_step,
                                 self._spec.begin_pruning_step),
  ...
  def add_pruning_summaries(self):
    ...
    with ops.name_scope(self._spec.name + '_summaries'):
      ...
      for mask, threshold, weight in zip(masks, thresholds, weights):
        if not self._exists_in_do_not_prune_list(mask.name):
          summary.scalar(mask.op.name + '/sparsity', nn_impl.zero_fraction(mask))
          summary.scalar(threshold.op.name, threshold)
          summary.histogram(weight.op.name, weight)
